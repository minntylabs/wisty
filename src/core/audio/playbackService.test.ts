...
describe("stopping and releasing", () => {
  ...
  it("reports a failed release rather than throwing into the close path", async () => {
    // release() is called while closing a document. Throwing there would stop
    // the user opening the next file over an audio device problem.
    const h = createHarness();
