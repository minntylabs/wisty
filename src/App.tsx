...
function App() {
  ...
  const receiveConversionOutput = (output: ConversionOutput) => {
    ...
    // watch takes lands after the conversion has been cleared away, and letting
    // that empty it would blank the window on its way out.
   ...
   * says another word, which re-creates the window's contents around whoever is
   * reading them.
   */
  const audioConversionProps = {
    get open() {
      return converting();
    },
    get lines() {
  ...
  const commandRegistry = createCommandRegistry(definitions, (commandId, error) => {
    const appError = toAppError(error, "UNKNOWN", "Unable to complete that action");
    errorModalQueue.enqueue({
      title: "Unable to complete that action",
      message: appError.message,
      code: appError.code,
      details: { ...(appError.details ?? {}), command: commandId }
    });
  });
    ...
    <CommandsProvider value={commandsContextValue}>
      <MenuProvider value={menuContextValue}>
        <AppShell
          ...
          audioConversion={audioConversionProps}
          importProblems={{
            get open() {
